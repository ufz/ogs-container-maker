...
for build in c:
    __format = build[0]
    ogs_version = build[1]
    package_manager.set(pm=build[2])
    ompi = build[3]
    ...
    img_file = f"{name_image}-{name_start}-{name_openmpi}-{package_manager.get().name.lower()}"
    img_folder = f"{name_image}/{name_start}/{name_openmpi}/{package_manager.get().name.lower()}"
    if len(cmake_args) > 0:
    ...
    if package_manager.get() == package_manager.CONAN:
        Stage0 += pm_conan()
        if not args.jenkins:
